...
(\registers,\inputsignals,\assignments,\init,\concexecution)$, which is defined as
follows.
\begin{itemize}
...
\item $\assignments \subseteq \registers \cup \inputsignals \times \{0,1\}^d$
  is the finite set of assignments---valuations---to the state and input
  variables. An assignment  to a
  ...
  sense as the length or dimension of hte design. We sometimes use
  $\assignments_{\registers}$ or $\assignments_{\inputsignals}$ to refer to the
  mapping of bitvector values to just the state holding registers or just the
...
\item $\init \in \assignments_{\registers}$ is the valuation to state-holding
  ...
  completes. I.e., $\init$ represents the initial state of the hardware
  module. \cks{Make sure the background section on RTL HW design describes
    initialization.} \cks{The coq model doesn't include an init state? Double
    check.}
\item $\concexecution: \assignments_{\registers} \times
  \assignments_{\inputsignals} \rightarrow \assignments_{\registers}$ is the
  transition function that represents concrete execution of hte hardware
  module. 
\end{itemize}
