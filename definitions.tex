...
We model the RTL design as a Coq program with the following types and function.
\begin{itemize}
\item Types
  \begin{itemize}
...
\item Function
  \begin{itemize}
  \item $\delta$ : (State, Input) : State
  \end{itemize}
\end{itemize}
       
\cks{A state is represented as a list of state tuples}


We build a Coq model of a hardware design that represents the stateful
...
state is given by concrete valuations \cks{Ar? In Coq concrete execution model
  introduce the notion of list of tuples <(),(),...>}. For symbolic execution we
replace concrete valuations \cks{Ar?} with symbolic expressions
...
replaced with symbolic expressions as well \cks{Ir?}
...
\begin{itemize}
\item $\symstates = $\cks{figure this out}
\item $\syminputs =$\cks{figure this out}
\item $\trees =$\cks{figure this out}.
\end{itemize}
...
\cks{Define expressions \symexpression{} in terms of symbols $\alpha$,
  etc. Define symbolic state \symstate{} in terms of expressions. Ditto for path
