...
The recursive search strategy works to find $n$ inputs that will take the hardware module
 from an initial reset state to an error state in $n$ clock cycles. 
...
This search continues until one of the found $s_i$'s is a reset state. 
...
Each application of symbolic execution produces a tree with a particular
leaf node of interest---the desired next-state $s_i$. 
...
\subsubsection{Three Original Requirements} This sequence of trees ($\mathit{tree\_list}$)
